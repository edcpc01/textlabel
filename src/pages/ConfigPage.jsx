...
export function ConfigPage() {
  const [empresa, setEmpresaLocal] = useState({ nome: '', cnpj: '', linha: '' })
  const [printer, setPrinter] = useState({ vel: '3', dens: '15', offx: '0' })
  const [ciclos,  setCiclos]  = useState([])
  const [editando, setEditando] = useState(null)  // { lote, maquina, valor }
  const [loading, setLoading] = useState(false)

  useEffect(() => {
    getEmpresa().then(d => {
      if (d?.nome) setEmpresaLocal({ nome: d.nome||'', cnpj: d.cnpj||'', linha: d.linha||'' })
  ...
  return (
    <div className="page-wrap">
      <div className="page-header">
        ...
        <p className="page-subtitle">Empresa · Impressora · Ciclos por Lote/Máquina</p>
      </div>
        ...
        <div className="card-body">
          <div className="form-grid">
            <div className="form-group span2">
              ...
              <input className="form-control" placeholder="Nome da Empresa" value={empresa.nome}
                onChange={e => setEmpresaLocal(p => ({ ...p, nome: e.target.value }))} />
            </div>
            <div className="form-group">
              <label className="form-label">CNPJ</label>
              <input className="form-control" placeholder="00.000.000/0000-00" value={empresa.cnpj}
                onChange={e => setEmpresaLocal(p => ({ ...p, cnpj: e.target.value }))} />
            </div>
            <div className="form-group">
              <label className="form-label">Linha de Produção</label>
              <input className="form-control" placeholder="Linha 1" value={empresa.linha}
                onChange={e => setEmpresaLocal(p => ({ ...p, linha: e.target.value }))} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div className="card-body">
          ...
          <div style={{ marginTop: 16 }}>
            <button className="btn btn-primary btn-sm" onClick={salvarConfig} disabled={loading}>
              <Save size={13} /> {loading ? 'Salvando...' : 'Salvar Configurações'}
            </button>
          </div>
        </div>
      </div>

      {/* CICLOS POR LOTE + MÁQUINA */}
      <div className="card">
