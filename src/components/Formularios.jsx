...
export function gerarEImprimirFormularios(dados) {
  ...
  // Extrai torção da composição ou descrição (SO, S, Z, etc.)
  const torcao = (() => {
  ...
  // Título dtex
  const tituloStr = titulo ? `${titulo}TEX` : ''

  // ── Gera linhas da tabela de fusos para formulário 2 ──
  const metade    = Math.ceil(totalFusos / 2)
  const fusoLinhas1 = Array.from({ length: metade }, (_, i) => i + 1)
  const fusoLinhas2 = Array.from({ length: totalFusos - metade }, (_, i) => metade + i + 1)

  const rowsFuso1 = fusoLinhas1.map(f =>
    `<tr><td>${f}</td><td></td><td></td></tr>`
  ).join('')
  const rowsFuso2 = fusoLinhas2.map(f =>
    `<tr><td>${f}</td><td></td><td></td></tr>`
  ).join('')
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* ───────────── FORMULÁRIO 1 FRENTE ───────────── */
  .f1 { display:flex; flex-direction:column; gap:4mm; }
  ...
  .ciclo-box .num { font-size:44pt; font-weight:900; letter-spacing:1px; line-height:1; }
  .ciclo-box .lbl { font-size:10pt; font-weight:700; margin-bottom:2mm; }
  .barcode { border:1.5px dashed #aaa; width:48mm; height:20mm; display:flex; align-items:center; justify-content:center; font-size:7pt; color:#aaa; text-align:center; }
  ...
  .cl-linha:last-child { margin-bottom:0; }
  ...
  /* ───────────── FORMULÁRIO 1 VERSO ───────────── */
  .f1v { display:flex; flex-direction:column; gap:0; }
  ...
  .instrucao { display:flex; align-items:center; justify-content:center; text-align:center; font-size:8.5pt; font-weight:700; padding:3mm; line-height:1.6; background:#f5f5f5; }
  ...
  /* ───────────── FORMULÁRIO 2 ───────────── */
  .f2 { display:flex; flex-direction:column; gap:3mm; }
...
<!-- ═══════════════════════════════════════════════════
     PÁGINA 1 — FRENTE DO FORMULÁRIO DE CICLO
═══════════════════════════════════════════════════ -->
<div class="pagina">
<div class="f1">
  ...
  <div class="grid2">
    <div class="campo">
      ...
      <div class="v">${empresa || ''}</div>
    </div>
    <div class="campo">
      <label>Lote</label>
      <div class="v">${lote}</div>
    </div>
  </div>

  <div class="campo">
    <label>Descrição do Material</label>
    <div class="v lg">${descricao}</div>
  </div>
  ...
  <div class="ciclo-box">
    ...
    <div class="barcode">[código de barras]<br>* ${maqCiclo} *</div>
  </div>

  <div class="grid3">
    <div class="campo">
      <label>Máquina</label>
      <div class="v">${maquina}</div>
    </div>
    <div class="campo">
      <label>Turno</label>
      <div class="v">&nbsp;</div>
    ...
    <div class="campo">
      <label>Data / Hora Ciclo</label>
      <div class="v">${dataHora}</div>
    </div>
  </div>
...
<!-- ═══════════════════════════════════════════════════
     PÁGINA 2 — VERSO (DEFEITOS DE ESCOLHA VISUAL)
═══════════════════════════════════════════════════ -->
<div class="pagina">
<div class="f1v">
  ...
  <div class="grade">
    ...
    <div class="cat ultima">
      <div class="cat-titulo">Bobinas com Anel</div>
      <div class="celulas">${Array(16).fill('<div class="cel"></div>').join('')}</div>
    </div>
    <div class="cat ultima">
      <div class="cat-titulo">Bobinas Batidas</div>
      <div class="celulas">${Array(16).fill('<div class="cel"></div>').join('')}</div>
    </div>
    <div class="cat ultima">
      <div class="cat-titulo">&nbsp;</div>
      <div class="celulas">${Array(16).fill('<div class="cel"></div>').join('')}</div>
    </div>
  ...
  <table class="tabela-rodape">
    <thead>
      <tr><th style="width:28mm;"></th><th style="width:35mm;">Data</th><th style="width:40mm;">Turma</th><th>Responsável</th></tr>
    </thead>
    <tbody>
...
<!-- ═══════════════════════════════════════════════════
     PÁGINA 3 — CLASSIFICAÇÃO VISUAL DE AFINIDADE TINTORIAL (PAISAGEM)
═══════════════════════════════════════════════════ -->
<div class="pagina" style="page-break-before:always;">
<style>
  @page :nth(3) { size: A4 landscape; }
</style>
<div class="f2">
  ...
  <div class="dados-linha dl2">
    <div class="celula"><label>DataHoraCiclo</label><div class="cv">${dataHora}</div></div>
