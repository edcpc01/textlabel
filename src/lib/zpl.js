// src/lib/zpl.js
// Zebra ZT230 — 200dpi — 50×30mm por etiqueta — 2 colunas × 3 linhas
// H=236 dots por etiqueta | W=393 dots por coluna
...
function blocoEtiqueta(record, ox, oy, L) {
  ...
  const mT  = Number(L.margemTop)   || 4
  const mX  = Number(L.margemX)     || 8

  // Largura interna (sem margem lateral)
  const W   = 393 - mX * 2
  const xL  = ox + mX
  ...
  // ── Y dinâmico — acumula de cima para baixo ──
  let y = oy + mT
  ...
  // Valores Máq/Ciclo/Fuso — usa a maior altura
  ...
  // Lote/Data — empurra para baixo se sobrar espaço, limitado a 4 dots do fim
  const H_ETQ = 236
